...
$(() => {
  ...
  $('.create-form').on('submit', event => {
    ...
    const burger = $('#burgercreate').val().trim()

    const letterRe = /[A-z\s]/gi// RegExp to make sure only letters with burger in the name are allowed to be stored
    const burgerRe = /(?:burger)/i
    const notWord = /\d|[^a-z\s]/gi
